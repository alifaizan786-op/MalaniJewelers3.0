...
SELECT  Styles.Code
       ...
       Styles.StyleLongDesc AS BodyHTML,

       ,Styles.SKUCode
       ,Styles.SubCatCode
       ,CONCAT( CASE WHEN CSC3.CatSubCat IS NOT NULL THEN CSC3.CatSubCat ELSE '' END,CASE WHEN CSC3.CatSubCat IS NOT NULL THEN '->' ELSE '' END,CASE WHEN CSC2.CatSubCat IS NOT NULL THEN CSC2.CatSubCat ELSE '' END,CASE WHEN CSC2.CatSubCat IS NOT NULL AND CatSubCats.CatSubCat IS NOT NULL THEN '->' ELSE '' END,COALESCE(CatSubCats.CatSubCat,'') ) AS CategoryHierarchy
...
FROM Styles
LEFT JOIN CatSubCats
ON Styles.SubCatCode = CatSubCats.Code -- Level 1 
LEFT JOIN CatSubCats AS CSC2
ON CatSubCats.ParentCode = CSC2.Code -- Level 2 
LEFT JOIN CatSubCats AS CSC3
ON CSC2.ParentCode = CSC3.Code -- Level 3 
LEFT JOIN Vendors
ON Styles.VendCode = Vendors.Code
LEFT JOIN ClassCodes
ON Styles.ClassCode = ClassCodes.Code
LEFT JOIN CommonMastersData AS GoldKarat
ON Styles.GoldKt = GoldKarat.Code
LEFT JOIN CommonMastersData AS Color
ON Styles.Color = Color.Code
WHERE Styles.StockQty = 1
AND Styles.Hidden = 0
AND Styles.Purchasable = 1
AND Styles.AttribField141 IS Null
AND MultiSkuCode IS NUll
